...
class DeviceState(object):
    ...
    def get_semantic_info_tree(self):
        """
        构建语义树：提取当前页面中所有控件的语义信息，并以树结构显示
        """
        if not self.views:
            return "No views available."
        def get_tree_by_recursion(view, indent_level=0):
            """
            递归构建语义树
            """
            # exclude if the widget is covered
            if self.__safe_dict_get(view, 'covered'):
                return ""
            # exclude navigation bar if exists
            if (
                    self.__safe_dict_get(view, 'enabled') and self.__safe_dict_get(view, 'visible') 
                    and self.__safe_dict_get(view, 'resource_id') not in ['android:id/navigationBarBackground', 'android:id/statusBarBackground']
                    and self.__safe_dict_get(view, "package") != "com.ohos.sceneboard"
            ):
                indent = "  " * indent_level
                ...
                selected = view.get('selected', False)
                
                # 构建基础信息
                desc_parts = []
                if class_name:
                    desc_parts.append(f'class_name="{class_name}" ')
                if text:
                ...
                if len(desc_parts) == 0:
                    desc_parts.append("unknown widget")

                # 属性集合
                props = []
                if clickable: props.append("clickable")
                if scrollable: props.append("scrollable")
                if checkable: props.append("checkable")
                if long_clickable: props.append("long_clickable")
                if editable: props.append("editable")
                if checked: props.append("checked")
                if selected: props.append("selected")
                ...
                for child_id in children_ids:
                    ...
                    child_desc = get_tree_by_recursion(child_view, indent_level + 1)
                    if child_desc:
                        child_lines.append(child_desc)
        ...
        for v in self.views:
            if v.get("parent") == -1:
                ...
                break
        if not root_view:
            return "No root view found."
        return get_tree_by_recursion(root_view)

    def get_event_desc(self, event):
        """
        获取事件的详细描述
        """
        # 处理回退事件go back
        if isinstance(event, KeyEvent):
            desc = f'go {event.name.lower()}'
            return desc
        ...
        if isinstance(event, UIEvent):
            ...
            if class_name:
                class_name = class_name.split('.')[-1]
            content_desc = self.__safe_dict_get(view, 'content_description')
            text = self.__safe_dict_get(view, 'text')
            ...
            if content_desc:
                ...
                desc_parts.append(f'content_description="{content_desc}"')
            if resource_id:
                desc_parts.append(f'resource_id="{resource_id}"')
            if len(desc_parts) > 0:
                view_desc = "with " + ", ".join(desc_parts)
            if len(desc_parts) == 0:
                view_desc = "that is unkonwn"

        event_name = event.event_type
        if isinstance(event, SetTextEvent):
            event_name = f'enter "{event.text}" into'
        elif isinstance(event, ScrollEvent):
            event_name = f'scroll {event.direction.lower()}'
        elif isinstance(event, SetTextAndSearchEvent):
            event_name = f'search "{event.text}"'
            
        if isinstance(event, UIEvent):
            if view_status == "":
                event_desc = f'{event_name} the view {view_desc}'
            else:
                event_desc = f'{event_name} the {view_status} view {view_desc}'
        else:
            event_desc = f'{event_name}'
        return event_desc
